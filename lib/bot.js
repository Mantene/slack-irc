...
//  this.slack.rtm.on(RTM_EVENTS.REACTION_ADDED, message => {
//    const newreaction = JSON.stringify(message)
//    logger.debug(newreaction)
//    const { dataStore } = this.slack.rtm;
//    logger.debug(ircChannel)
...
//      this.ircClient.say(ircChannel, `${user.name} had the following reaction: ${newreaction}`);
//      logger.debug(newreaction)
//  }
//  })
    ...
    // Ignore bot messages and people leaving/joining
      ...
      message.item.subtype = 'me_message'
      ...
      if (message.type === 'reaction_added') {
        ...
        if (!message.subtype || ALLOWED_SUBTYPES.indexOf(message.subtype) > -1) {
          if (CODE_REGEX.test(message.text)) {
            ...
            message.c1 = message.item.channel
            message.u1 = message.user
            delete message.user
            delete message.item
            this.sendToIRC(message);
            logger.debug(`7`);
    ...
    // Ignore bot messages and people leaving/joining
      ...
      if (message.hasOwnProperty("topic")) {
        ...
        logger.debug(`Requesting topic for IRC channel ${ircChannel}`,`MESSAGE.TOPIC: ${message.topic}, MESSAGE.USER: ${message.user}`);
 
        this.ircClient.send('TOPIC', ircChannel, `${message.topic}`);
      }
      if (message.type === 'message') {
  ...
  sendToIRC(message) {
    logger.debug(`INSIDE SENDTOIRC FUNCTION`);
    logger.debug(message)
    if (message.type === 'reaction_added') {
      message = {'type': message.type, 'channel': message.c1, 'user': message.u1, 'text': `reacted to a previous post with :${message.reaction}:`, 'ts': message.ts, 'subtype': message.subtype}
    }
    logger.debug(message)
    const { dataStore } = this.slack.rtm;
    const channel = dataStore.getChannelGroupOrDMById(message.channel);
